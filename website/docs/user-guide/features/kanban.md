...
### Accepted content awaiting publication

For a prepared repository card, the independent reviewer can call `kanban_complete` with `metadata.review_outcome` set to `accepted_pending_publication`, the submitted `metadata.reviewed_state_id`, non-empty `metadata.reviewer_checks` and `metadata.publication.deliverable` set to `branch`, `pull_request` or `merge`. A summary and distinct developer/reviewer session identities are required. This ends the reviewer attempt with `publication_pending`, records the accepted content, returns the card to its developer in Todo and clears execution authority. The card remains unfinished. Notifications distinguish acceptance from content defects. Use `kanban_request_changes` for defects.

The operator records the task's actual approval through `authorise`, supplying `--authority` and repeatable `--publication-action` values from `commit`, `push`, `pull_request`, `merge` and `verify`. Existing explicit approval for an unused exact action can be recorded here. The operation records authority, it does not obtain approval from the user or perform Git writes. It reconciles local HEAD, remote branch and PR receipts, removes completed actions from the outstanding list and preserves the registered workspace. Unknown or conflicting receipts require reconciliation. An inactive completed card with recorded independent acceptance can enter later authorised publication through this same operation, preserving earlier attempts. Running dependent work must be paused before reopening its parent.

Publication preserves the accepted file contents, file modes, workspace, branch and task requirements. HEAD and index changes require a fresh developer submission for published-state review. Changed content, including a modification made by a Git hook, requires fresh content acceptance and scoped publication authority. Publication permission does not authorise implementation corrections. After a failed attempt, use `recover` before recording continuation authority. Recovery reconciles consumed actions. An interrupted published-state reviewer can resume Review with explicit review-continuation authority.

The active independent reviewer completes the fresh submission with `review_outcome` set to `approved`. Completion checks the exact submitted state, the accepted content and a clean checkout, then reads the promised remote deliverable. A branch requires a matching remote HEAD. A PR requires a matching open PR. A merge requires a matching merged PR, including when its source branch has been deleted. The run retains worker checks separately from the remote publication receipt. GitHub check receipts distinguish reported, absent and unavailable data. The [repository CI policy and merge preflight](kanban-ci.md) records repository-specific requirements and decisions before an authorised merge. These are point-in-time checks and task workflow records, not command interception or continuous remote monitoring.
...
- **Workspace** — the directory a worker operates in. Three kinds:
  ...
  - `worktree`: a retained linked Git worktree for coding tasks. Preparation records the repository, exact base, branch and path on the card. Developer and reviewer runs reuse that registered workspace. Missing or inconsistent workspaces require reconciliation before dispatch.

    Completion, publication, archive and parent/child cleanup preserve the worktree, including ignored evidence. Preparation adds a persistent Git lock and preserves an existing lock. Terminal cards retain their ownership and review records. Card or board deletion is refused while it would erase worktree ownership. An operator must use a separately authorised disposal procedure to release retained work.

    Reopening completed work clears its execution authority. The operator must authorise the continuation before another attempt can claim the card. Review corrections keep the card and workspace, and changed content requires a fresh independent review.
...
Worktree paths must be absolute and must not traverse symbolic links or `..` components.
Worktree task identifiers must be safe single path components of up to 128 characters, using letters, digits, underscores and hyphens.
The first character must be a letter or digit.
Concurrent workspace requests use a shared lock in the repository's common Git directory.
A request fails explicitly if it cannot acquire that lock within 120 seconds.
