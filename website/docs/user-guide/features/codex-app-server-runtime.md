...
These three Hermes tools require the running AIAgent context (mid-loop state) to dispatch, and a stateless MCP callback can't drive them. Switch back to the default runtime (`/codex-runtime auto`) when you need any of them:
...
| `session_search`, `todo` | yes | not available — needs agent loop context |
| `memory` | yes | yes (built-in stores via MCP) |
...
The MCP `memory` tool adds, replaces and removes entries in the selected profile's built-in stores.
It uses Hermes' storage limits and write approval gate. A staged result requires approval before the fact is saved.
The MCP registration inherits `HERMES_HOME` when each session launches.
Existing installations need the updated MCP registration to use this profile routing.
These operations cover built-in memory, not external memory-provider synchronisation.
...
When the model calls `web_search` (or another exposed Hermes tool), codex spawns the `hermes_tools_mcp_server` subprocess via stdio, the server dispatches the request, and the result returns to Codex as an MCP response.

**Tools available via the callback:** `web_search`, `web_extract`, `browser_navigate`, `browser_click`, `browser_type`, `browser_press`, `browser_snapshot`, `browser_scroll`, `browser_back`, `browser_get_images`, `browser_console`, `browser_vision`, `vision_analyze`, `image_generate`, `skill_view`, `skills_list`, `text_to_speech`, `memory`.

**Tools NOT available:** `delegate_task`, `session_search`, `todo`. These need the running AIAgent context to dispatch (mid-loop state) and a stateless MCP callback can't drive them. Use the default Hermes runtime (`/codex-runtime auto`) when you need these.
...
- **`delegate_task`, `session_search`, `todo` are unavailable on this runtime.** They need the running AIAgent context which a stateless MCP callback can't provide. Use `/codex-runtime auto` when you need these.
